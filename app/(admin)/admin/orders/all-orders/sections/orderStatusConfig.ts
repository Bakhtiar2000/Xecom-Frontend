...
export const isValidOrderStatusSegment = (value: string): value is TOrderStatusSegment => {
  return ORDER_STATUS_ROUTES.some((route) => route.segment === value);
};

export const getOrderStatusRouteConfig = (value: TOrderStatusSegment): TOrderStatusRouteConfig => {
  return ORDER_STATUS_ROUTES.find((route) => route.segment === value) || ORDER_STATUS_ROUTES[0];
};
