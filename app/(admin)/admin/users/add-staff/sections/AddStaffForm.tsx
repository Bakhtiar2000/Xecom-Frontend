...
export default function AddStaffForm() {
  ...
  const onSubmit = async (data: TRegisterStaffDto) => {
    try {
      ...
      const res = await registerStaff(formData as unknown as TRegisterStaffDto).unwrap();
      ...
      <div className="w-full rounded-2xl border bg-white p-8 shadow">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div>
              <Label>Name *</Label>
              <Input {...register("name")} placeholder="Staff name" required />
            </div>
            <div>
              <Label>Email *</Label>
            ...
            <div>
              ...
              <Input type="date" {...register("hireDate")} required />
            </div>
          </div>
