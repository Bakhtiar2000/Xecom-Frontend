        ...
        <div className="max-h-[calc(100vh-10rem)] overflow-y-auto px-6 py-4">
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            {[
              ...
              {
                label: "Avg rating",
                value: product.avgRating ? Number(product.avgRating).toFixed(1) : "0",
              },
            ].map(({ label, value }) => (
          ...
          <Section title="Order & stock">
            <div className="grid grid-cols-2 gap-2">
              ...
              <Field label="Max order qty" value={product.maxOrderQty ?? "0"} />
              <Field
                label="Weight"
                value={product.weight ? `${product.weight} ${product.weightUnit ?? ""}` : "0"}
              />
            ...
            <Section title="Tags">
              <div className="flex flex-wrap gap-1.5">
                {product.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-muted-foreground bg-muted rounded-full border px-2.5 py-0.5 text-xs"
                  >
                    {tag}
                  </span>
                ))}
          ...
          <Section title="SEO">
            <div className="grid grid-cols-2 gap-2">
              ...
              <Field label="Keywords" value={product.metaKeywords?.join(", ") || "—"} />
            </div>
            <h1 className="mt-2">
              {" "}
