...
export default function DetailsTab({ form }: DetailsTabProps) {
  ...
  const removeTag = (i: number) =>
    form.setValue("tags", form.getValues("tags").filter((_, idx) => idx !== i));
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-4">
              {(["width", "height", "length"] as const).map((dim) => (
                <FormField key={dim} control={form.control} name={`dimensions.${dim}`} render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Input
                        type="number"
          ...
          <div className="space-y-2">
            ...
            {form.formState.errors.tags && (
              <p className="text-sm text-danger">{form.formState.errors.tags.message}</p>
            )}
          ...
          {/* Order Qty */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField control={form.control} name="minOrderQty" render={({ field }) => (
              <FormItem>
            ...
            <FormField control={form.control} name="maxOrderQty" render={({ field }) => (
              <FormItem>
