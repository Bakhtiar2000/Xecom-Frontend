...
const SingleAttributeFilter = ({
  ...
  const selectedValuesForThisAttr = selectedValueIds.filter((id) => attributeValueIds.includes(id));
  ...
  const selectedItems = (attribute.values || []).filter((v) => selectedValueIds.includes(v.id));
...
export const AttributeValueFilter = ({
  ...
  if (loading) {
    return <div className="text-muted-foreground py-2 text-sm">Loading attributes...</div>;
  }

  if (attributes.length === 0) {
    return <div className="text-muted-foreground py-2 text-sm">No attributes available</div>;
  }
